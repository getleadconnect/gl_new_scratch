...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-muted-foreground">View your purchase history (last 3 months by default)</p>
        </div>
    </div>
...
$(document).ready(function() {
    var table = $('#purchase-history-table').DataTable({
        ...
        ajax: {
            url: "{{ route('user.purchase-history.data') }}",
            type: 'GET',
            data: function(d) {
    ...
    $('#btn-export-csv').on('click', function() {
        ...
        window.location.href = "{{ route('user.purchase-history.export') }}" + '?' + params;
    });
